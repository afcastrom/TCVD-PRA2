...
# 4.3.1 Existe diferencia en el precio entre los A3s Gasolina y los Diesel...?


--AÑADIR Toni

Contraste de Hipotesis bilateral; datos independientes, ordenables y procedentes de muestras no Normales y con igualdad de varianzas. 

Decidimos representar los datos 
...
Y aplicar el test no parametrico Mann–Whitney–Wilcoxon.
```{r}
...
Como el p value > 0.05 ; NO rechazamos la hipotesis nula; es decir no hay diferencias significativas entre los precios de los vehiculos diesel y los gasolina. 

Hecho tambien observado comparando los diagramas de cajas de ambos.



# 4.3.2 Estudi de variables quantitatives que influeixen en el preu. 
...
Realitzarem un model de regressió amb les variables quantitatives i cualitatives. I un altre nomès amb les quantitatives. Prèviament, deduim de les correlacions observades, que no hi ha multicolinealitat amb les variables.
```{r}
...
Observem que les tres variables cualitatives practicament NO influeixen en el preu. De fet nomès mijora el coeficient de determinacio en 3 punts.


Observem que les tres variables cuantitatives SI influeixen en el preu.El p valor és molt petit per a les 3 variables.



# 4.3.3 Construcció model regressió de variables quantitatives.
